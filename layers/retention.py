# !/usr/bin/env python3
"""
copy form https://github.com/Jamie-Stirling/RetNet
"""
import math
...
class SimpleRetention(nn.Module):
    ...
    def forward_chunkwise(self, x_i, r_i_1, i):
        ...
        r_i = (K.transpose(-1, -2) @ (V * D[-1].view(1, chunk_size, 1))) + (
            self.gamma**chunk_size
        ) * r_i_1
...
class MultiScaleRetention(nn.Module):
    def __init__(self, hidden_size: int, heads: int, double_v_dim: bool = False):
        """
...
class RetentionBlock(nn.Module):
    def __init__(self, ffn_size:int, hidden_dim:int, num_heads:int, double_v_dim:bool=False) -> None:
        super(RetentionBlock, self).__init__()

        self.retention = MultiScaleRetention(hidden_dim, num_heads, double_v_dim)

        self.ffns = nn.Sequential(
            nn.Linear(hidden_dim, ffn_size), nn.GELU(), nn.Linear(ffn_size, hidden_dim)
        )
    ...
    def forward(self, x):
        x = self.retention(self.norm1(x)) + x

        x = self.ffns(self.norm2(x)) + x
        return x
